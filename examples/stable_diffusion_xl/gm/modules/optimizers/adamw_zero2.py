...
class AdamWeightDecayZeRO2(Optimizer):
    ...
    def __init__(
        ...
        self._opt_params_need_offload = {
            ...
            "beta2": self.beta2,
            "eps": self.eps,
            "moments1": self.moments1,
            "moments2": self.moments2,
        }
    ...
    def _init_momentum(self, params, prefix, init="zeros"):
        ...
        for i, param in enumerate(params):
            ...
            moment = ms.Parameter(
                initializer(init, shape=param_shape, dtype=mstype.float32), name=prefix + "." + param.name
            )
    ...
    def construct(self, grads):
        """construct method"""
        params = self.hyper_map(
            F.partial(_split_params, self.shard_id, self.shard_size), self._parameters, self._parameter_splited
        )
