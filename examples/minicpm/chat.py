...
def init_omni_lmm(model_path):
    ...
    if False:
        ...
        with init_empty_weights():
            model = OmniLMMForCausalLM.from_pretrained(model_name, tune_clip=True, mindspore_dtype=ms.bfloat16)
        model = load_checkpoint_and_dispatch(model, model_name, dtype=ms.bfloat16,
                    device_map="auto",  no_split_module_classes=['Eva','MistralDecoderLayer', 'ModuleList', 'Resampler']
        )
    else:
        model = OmniLMMForCausalLM.from_pretrained(
            model_name, tune_clip=True, torch_dtype=ms.bfloat16
        ).to(device='cuda', dtype=ms.bfloat16)
...
class MiniCPMV:
    def __init__(self, model_path) -> None:
        self.model = MiniCPM_model.from_pretrained(model_path, trust_remote_code=True).to(dtype=ms.bfloat16)
...
class MiniCPMV2_5:
    def __init__(self, model_path) -> None:
        self.model = MiniCPM_model.from_pretrained(model_path, trust_remote_code=True).to(dtype=ms.float16)
        self.tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)
        self.model.eval().cuda()
