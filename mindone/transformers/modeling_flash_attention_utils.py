...
def _get_unpad_data(attention_mask: ms.Tensor) -> Tuple[ms.Tensor, ms.Tensor, int]:
    ...
    """
    seqlens_in_batch = attention_mask.sum(axis=-1, dtype=ms.int32)
    indices = ops.nonzero(attention_mask.flatten(), as_tuple=False).flatten()
    max_seqlen_in_batch = seqlens_in_batch.max().item()
    cu_seqlens = F.pad(ops.cumsum(seqlens_in_batch, axis=0, dtype=ms.int32), (1, 0))
    return (
        indices,
        cu_seqlens,
        max_seqlen_in_batch,
...
def prepare_fa2_from_position_ids(query, key, value, position_ids):
    ...
    indices_q = ops.arange(position_ids.size(0), dtype=ms.int32)
