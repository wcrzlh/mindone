...
class RTDetrHungarianMatcher(nn.Cell):
    ...
    def construct(self, outputs, targets):
        ...
        if self.use_focal_loss:
            out_prob = ops.sigmoid(outputs["logits"].flatten(0, 1))
...
class RTDetrLoss(nn.Cell):
    ...
    def __init__(self, config):
        ...
        empty_weight = ops.ones(config.num_labels + 1)
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_boxes = ops.cat([_target["boxes"][i] for _target, (_, i) in zip(targets, indices)], axis=0)
        ...
        target_score_original = ops.zeros_like(target_classes, dtype=src_logits.dtype)
        target_score_original[idx] = ious.to(target_score_original.dtype)
        target_score = target_score_original.unsqueeze(-1) * target

        pred_score = ops.sigmoid(src_logits)
        weight = self.alpha * pred_score.pow(self.gamma) * (1 - target) + target_score

        loss = ops.binary_cross_entropy_with_logits(src_logits, target_score, weight=weight, reduction="none")
    ...
    def loss_labels_bce(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target = ops.one_hot(target_classes, depth=self.num_classes + 1)[..., :-1]
        loss = ops.binary_cross_entropy_with_logits(src_logits, target * 1.0, reduction="none")
        loss = loss.mean(1).sum() * src_logits.shape[1] / num_boxes
    ...
    def get_cdn_matched_indices(dn_meta, targets):
        ...
        for i, num_gt in enumerate(num_gts):
            if num_gt > 0:
                gt_idx = ops.arange(num_gt, dtype=ms.int64)
    ...
    **kwargs,
    ...
    if config.auxiliary_loss:
        if denoising_meta_values is not None:
            dn_out_coord, outputs_coord = ops.split(outputs_coord, denoising_meta_values["dn_num_split"], axis=2)
            dn_out_class, outputs_class = ops.split(outputs_class, denoising_meta_values["dn_num_split"], axis=2)
