...
class ZeroShotClassificationPipeline(ChunkPipeline):
    ...
    **much** more flexible.
    ...
    def _forward(self, inputs):
        ...
        model_forward = self.model.construct if self.framework == "ms" else self.model.call
    ...
    def postprocess(self, model_outputs, multi_label=False):
        ...
        if self.framework == "ms":
            logits = np.concatenate([output["logits"].float().asnumpy() for output in model_outputs])
        else:
            logits = np.concatenate([output["logits"].asnumpy() for output in model_outputs])
        N = logits.shape[0]
