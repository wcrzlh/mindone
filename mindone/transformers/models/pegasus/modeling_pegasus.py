...
# Copied from mindone.transformers.modeling_attn_mask_utils.dtype_to_max
...
class PegasusSinusoidalPositionalEmbedding(nn.Embedding):
    ...
    def _init_weight(out: Parameter) -> Parameter:
        ...
        out[:, 0:sentinel] = ms.Tensor(np.sin(position_enc[:, 0::2]), dtype=ms.float32)
        out[:, sentinel:] = ms.Tensor(np.cos(position_enc[:, 1::2]), dtype=ms.float32)
        out.detach_()
        return out
...
class PegasusPreTrainedModel(MSPreTrainedModel):
