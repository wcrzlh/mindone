...
class ZambaHybridDynamicCache(DynamicCache):
    ...
    def __init__(self, config, batch_size, dtype=ms.float16):
        ...
        for i in range(config.num_hidden_layers):
            self.conv_states += [
                mint.zeros((batch_size, self.intermediate_size, self.conv_kernel_size), dtype=dtype)
            ]
            cache_shape = (
                batch_size,
                self.n_mamba_heads,
...
class ZambaAttention(nn.Cell):
    ...
    def __init__(self, config: ZambaConfig, layer_idx: int):
        ...
        self.q_proj = mint.nn.Linear(config.attention_hidden_size, config.num_attention_heads * self.head_dim, bias=False)
        self.k_proj = mint.nn.Linear(config.attention_hidden_size, config.num_key_value_heads * self.head_dim, bias=False)
        self.v_proj = mint.nn.Linear(config.attention_hidden_size, config.num_key_value_heads * self.head_dim, bias=False)
    ...
    def cuda_kernels_forward(
        ...
        if use_precomputed_states:
            ...
            if cache_params is not None:
                conv_states = mint.nn.functional.pad(hidden_states, (self.conv_kernel_size - hidden_states.shape[-1], 0))
...
class ZambaHybridLayer(nn.Cell):
    def __init__(self, shared_transf: ZambaAttentionDecoderLayer, linear: mint.nn.Linear, mamba: ZambaMambaDecoderLayer):
        super().__init__()
