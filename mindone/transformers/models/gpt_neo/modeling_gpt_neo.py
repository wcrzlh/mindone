...
def load_tf_weights_in_gpt_neo(model, config, gpt_neo_checkpoint_path):
    ...
    lin = nn.Dense(embs.shape[1], embs.shape[0], has_bias=False)
    lin.weight = embs
...
class GPTNeoSelfAttention(nn.Cell):
    def __init__(self, config, attention_type, layer_id=None):
        ...
        bias = ops.tril(ops.ones((max_positions, max_positions), dtype=ms.bool_)).view(
        ...
        self.bias = bias
        self.masked_bias = ms.tensor(-1e9)
...
class GPTNeoPreTrainedModel(MSPreTrainedModel):
    """
...
class GPTNeoModel(GPTNeoPreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.CellList([GPTNeoBlock(config, layer_id=i) for i in range(config.num_layers)])
    ...
    def construct(
        ...
        past_key_values: Optional[Union[Cache, Tuple[ms.Tensor]]] = None,
