...
from transformers.models.glm4v_moe.configuration_glm4v_moe import Glm4vMoeConfig, Glm4vMoeTextConfig, Glm4vMoeVisionConfig


logger = logging.get_logger(__name__)


class Glm4vMoeRMSNorm(nn.Cell):
...
class Glm4vMoeTextAttention(nn.Cell):
...
class Glm4vMoeTextTopkRouter(nn.Cell):
    def __init__(self, config: Glm4vMoeTextConfig):
        ...
        self.e_score_correction_bias = mint.zeros((self.n_routed_experts), dtype=ms.float32)
...
class Glm4vMoeTextRMSNorm(nn.Cell):
    def __init__(self, hidden_size, eps=1e-6):
        """
...
class Glm4vMoeisionMlp(nn.Cell):
...
class Glm4vMoeVisionPatchEmbed(nn.Cell):
    def __init__(self, config: Glm4vMoeVisionConfig) -> None:
        ...
        self.proj = mint.nn.Conv3d(self.in_channels, self.embed_dim, kernel_size=kernel_size, stride=kernel_size)
...
class Glm4vMoeVisionRotaryEmbedding(nn.Cell):
...
class Glm4vMoeVisionPatchMerger(nn.Cell):
    def __init__(self, dim: int, context_dim: int, hidden_act: str, bias: bool = False) -> None:
        super().__init__()
        self.proj = mint.nn.Linear(dim, dim, bias=bias)
        self.post_projection_norm = mint.nn.LayerNorm(dim)
        self.gate_proj = mint.nn.Linear(dim, context_dim, bias=bias)
        self.up_proj = mint.nn.Linear(dim, context_dim, bias=bias)
        self.down_proj = mint.nn.Linear(context_dim, dim, bias=bias)
...
class Glm4vMoeVisionEmbeddings(nn.Cell):
    ...
    def construct(self, embeddings, lengths, image_shapes, h_coords, w_coords) -> ms.tensor:
        ...
        Args:
            ...
            lengths (ms.tensor): Sequence lengths for each image in the batch.
            image_shapes (ms.tensor): Tensor of shape [batch_size, 3] representing the image shapes (t, h, w).
            h_coords (ms.tensor): Tensor of shape [total_seq] representing the h coordinate for each patch.
            w_coords (ms.tensor): Tensor of shape [total_seq] representing the w coordinate for each patch.

        Returns:
            ms.tensor: Embeddings with adapted position encoding added.
        """
        ...
        if total_seq == 0:
            ...
            target_h = mint.cat([image_shapes[i, 1].repeat(lengths[i].item()) for i in range(len(lengths))]).to(
                dtype=ms.float32
            )
            target_w = mint.cat([image_shapes[i, 2].repeat(lengths[i].item()) for i in range(len(lengths))]).to(
                dtype=ms.float32
            )
            ...
            # fixme there is no bicubic mode in mint.nn.functional.grid_sample
            interpolated_embed_fp32 = mint.nn.functional.grid_sample(
...
class Glm4vMoeVisionAttention(nn.Cell):
    def __init__(self, config: Glm4vMoeVisionConfig) -> None:
        super().__init__()
        ...
        self.qkv = mint.nn.Linear(config.hidden_size, config.hidden_size * 3, bias=config.attention_bias)
        self.proj = mint.nn.Linear(config.hidden_size, config.hidden_size, bias=False)
        self.scaling = self.head_dim**-0.5
        self.config = config
        self.attention_dropout = config.attention_dropout
...
class Glm4vMoeTextRotaryEmbedding(nn.Cell):
    inv_freq: ms.tensor  # fix linting for `register_buffer`
...
class Glm4vMoeVisionModel(Glm4vMoePreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        self.blocks = nn.CellList([Glm4vMoeVisionBlock(config) for _ in range(config.depth)])
...
class Glm4vMoeModel(Glm4vMoePreTrainedModel):
    ...
    def get_video_features(
        ...
        for t, h, w in video_grid_thw:
            repeated_row = ms.tensor([1, h.item(), w.item()]).unsqueeze(0).repeat(t, 1)
            temp_frames_hw.append(repeated_row)
        flattened_video_grid_thw = mint.cat(temp_frames_hw, dim=0)
    ...
    def get_placeholder_mask(
        ...
        image_features: ms.tensor = None,
        video_features: ms.tensor = None,
    ):
        """
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if position_ids is None:
            ...
            if attention_mask_tensor is not None and attention_mask_tensor.ndim == 4:
                attention_mask_tensor = mint.nn.functional.diagonal(attention_mask_tensor[:, 0], dim1=1, dim2=2)
        ...
        if inputs_embeds is not None:
            is_image = (
                ...
                == self.get_input_embeddings()(
                    ms.tensor(self.config.image_start_token_id, dtype=ms.int64)
                )
            )[..., 0]
            is_video_start = (
                inputs_embeds
                == self.get_input_embeddings()(
                    ms.tensor(self.config.video_start_token_id, dtype=ms.int64)
                )
            )[..., 0]
            is_video_end = (
                inputs_embeds
                == self.get_input_embeddings()(
                    ms.tensor(self.config.video_end_token_id, dtype=ms.int64)
                )
            )[..., 0]
        else:
