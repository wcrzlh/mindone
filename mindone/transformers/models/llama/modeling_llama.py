...
class LlamaRotaryEmbedding(nn.Cell):
    ...
    def construct(self, x, position_ids):
        if "dynamic" in self.rope_type:
            self._dynamic_frequency_update(position_ids, device=None)
...
class LlamaMLP(nn.Cell):
    ...
    def construct(self, x):
        if self.config.pretraining_tp > 1:
            ...
            gate_proj = ops.cat(
                [F.linear(x, gate_proj_slices[i]) for i in range(self.config.pretraining_tp)], axis=-1
            )
...
class LlamaAttention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs,
        ...
        attn_weights = ops.matmul(query_states, key_states.permute(0 , 1, 3, 2)) / math.sqrt(self.head_dim)

        if attention_mask is not None:  # no matter the length, we just slice it
            causal_mask = attention_mask[:, :, :, 0:key_states.shape[-2]]
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def construct(
        ...
        attention_mask: Optional[ms.Tensor] = None,
        position_ids: Optional = None,
        past_key_values: Optional[Union[List[ms.Tensor]]] = None,
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def construct(
        ...
        if labels is not None:
            ...
            shift_labels = shift_labels.to(None)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if inputs_embeds is not None and cache_position[0] == 0:
            ...
            model_inputs = {"input_ids": input_ids}  # `` needed for compilation use cases
    ...
    def construct(
        ...
        if labels is not None:
            ...
            if self.config.problem_type is None:
                ...
                elif self.num_labels > 1 and (labels.dtype == ops.long or labels.dtype == ops.int):
...
class LlamaForQuestionAnswering(LlamaPreTrainedModel):
    ...
    def construct(
        ...
        past_key_values: Optional[Union[List[ms.Tensor]]] = None,
        inputs_embeds: Optional[ms.Tensor] = None,
        start_positions: Optional = None,
        end_positions: Optional = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
