...
class MiniCPMVProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        if images is not None:
            ...
            # image_inputs = MiniCPMVImageProcessor().preprocess(images=images, do_pad=do_pad, return_tensors=return_tensors)
    ...
    def _convert(
        ...
        # FIXME input ids issue
        # image_start_tokens = Tensor(image_start_tokens)
        # image_start_tokens = ops.where(input_ids == self.tokenizer.im_start_id, input_ids, self.tokenizer.im_start_id)[0]
        # image_start_tokens += 1
        # image_end_tokens = ops.where(input_ids == self.tokenizer.im_end_id, input_ids, self.tokenizer.im_end_id)[0]
        image_start_tokens = []
        for i in range(len(input_ids)):
            if input_ids[i] == self.tokenizer.im_start_id:
                image_start_tokens.append(i+1)
        image_start_tokens = Tensor(np.array(image_start_tokens))
        image_end_tokens = []
        for i in range(len(input_ids)):
            if input_ids[i] == self.tokenizer.im_end_id:
                image_end_tokens.append(i)
        # image_end_tokens = Tensor(image_end_tokens)
    ...
    def _convert_images_texts_to_inputs(self, images, texts, do_pad=False, truncation=None, max_length=None,
                                        return_tensors=None):
        if not len(images):
            model_inputs = self.tokenizer(texts, return_tensors=return_tensors, padding=do_pad, truncation=truncation,
        ...
        for i in range(len(image_tags)):
            final_texts = final_texts + text_chunks[i] + self.image_processor.get_slice_image_placeholder(
                image_sizes[0][i])
    ...
    def pad(self, orig_items, key, max_length=None, padding_value=0, padding_side="left"):
        ...
        if isinstance(orig_items[0][key], list):
            assert isinstance(orig_items[0][key][0], ms.Tensor)
            for it in orig_items:
                for tr in it[key]:
                    items.append({key: tr})
        else:
            assert isinstance(orig_items[0][key], ms.Tensor)
            items = orig_items
