...
class Resampler(nn.Cell):
    ...
    def construct(self, x, tgt_sizes=None):
        ...
        for i in range(bs):
            ...
            shape_0 = tgt_h * tgt_w
            pos_embed.append(self.pos_embed[:tgt_h, :tgt_w, :].reshape((int(shape_0.asnumpy()), -1)).to(dtype))  # patches * D
        ...
        for i in range(len(pos_embed)):
            ...
            0, max_length_w - pos_embed[i].shape[1], 0, max_length_h - pos_embed[i].shape[0]), value=0.0)
...
class MultiheadAttention(nn.MultiheadAttention):
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, 
        ...
        super().__init__(embed_dim, num_heads, dropout, bias, add_bias_kv, add_zero_attn, kdim, vdim, batch_first, dtype)
