...
class AxialPositionEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        for axis, axial_pos_embd_dim in enumerate(self.axial_pos_embds_dim):
            ...
            self.weights.append(Parameter(ops.ones(ax_shape, dtype=ms.float32)))
    ...
    def construct(self, input_ids=None, position_ids=None, inputs_embeds=None, start_idx_pos_encodings=0):
        ...
        if position_ids is None:
            position_ids = ops.arange(
                start_idx_pos_encodings, start_idx_pos_encodings + seq_length, dtype=ms.int64
            )
...
class LSHSelfAttention(nn.Cell, EfficientAttentionMixin):
    def __init__(self, config):
        ...
        self.self_mask_value_float16 = ms.tensor(-1e3)
        self.self_mask_value_float32 = ms.tensor(-1e5)
        self.mask_value_float16 = ms.tensor(-1e4)
        self.mask_value_float32 = ms.tensor(-1e9)
    ...
    def _hash_vectors(self, vectors, num_hashes, attention_mask, increase_num_buckets=False):
        ...
        if attention_mask is not None and (attention_mask.sum().item() < batch_size * attention_mask.shape[-1]):
            ...
            buckets_mask = attention_mask.to(ms.bool_)[:, None, None, :].expand(buckets.shape)
            buckets = ops.where(
                buckets_mask, buckets, ms.tensor(num_buckets - 1, dtype=ms.int64)
            )
    ...
    def _attend(
        ...
        if not do_standard_self_attention:
            ...
            key_value_bucket_idx = ops.arange(
                query_key_dots.shape[-1], dtype=ms.int64
            )[None, None, :].expand(query_bucket_idx.shape[:2] + (-1,))
        else:
            query_bucket_idx = key_value_bucket_idx = sorted_bucket_idx_per_hash

        # get correct mask values depending on precision
        if query_key_dots.dtype == ms.float16:
        ...
        self_mask = ops.ne(query_bucket_idx.unsqueeze(-1), key_value_bucket_idx.unsqueeze(-2)).to(
    ...
    def _compute_attn_mask(
        ...
        if attention_mask is not None:
            ...
            attention_mask = attention_mask.to(ms.bool_)[:, None, :]
            if not do_standard_self_attention:
    ...
    def _get_relevant_hid_states_and_buckets(
        ...
        offset = ops.arange(relevant_bucket_idx_chunk.shape[-1], dtype=ms.int64)
    ...
    def _expand_to_indices_in_relevant_chunk(self, indices, sequence_length):
        ...
        chunk_sequence_indices = expanded_start_indices + ops.arange(
            total_chunk_size, dtype=ms.int64
...
class ReverseSort(Function):
    ...
    @staticmethod
    def construct(ctx, out_vectors, logits, sorted_bucket_idx, undo_sorted_bucket_idx):
...
class ReformerLayer(nn.Cell):
    ...
    def _init_attention_seed(self):
        ...
        self.attention_seed = int(ms.seed() % sys.maxsize)

        ms.manual_seed(self.attention_seed)
    ...
    def _init_feed_forward_seed(self):
        ...
        # randomize seeds
        # use cuda generator if available
        self.feed_forward_seed = int(ms.seed() % sys.maxsize)

        ms.manual_seed(self.feed_forward_seed)
...
class _ReversibleFunction(Function):
    ...
    def backward(ctx, grad_hidden_states):
        grad_attn_output, grad_hidden_states = ops.chunk(grad_hidden_states, 2, axis=-1)
...
class ReformerPreTrainedModel(MSPreTrainedModel):
    """
...
class ReformerModelOutput(ModelOutput):
    ...
    past_buckets_states: Optional[List[Tuple[ms.Tensor, ms.Tensor]]] = None
...
class ReformerModel(ReformerPreTrainedModel):
    ...
    def construct(
        ...
        past_buckets_states: Optional[List[Tuple[ms.Tensor]]] = None,
    ...
    def _pad_to_mult_of_chunk_length(
        ...
        if attention_mask is not None:
            ...
            attention_mask = ops.cat(
                [
                    ops.ones(input_shape, dtype=ms.bool_),
                    ops.zeros((input_shape[0], padding_length), dtype=ms.bool_),
                ],
                axis=-1,
            )
        ...
        if input_ids is not None:
            ...
            if position_ids is not None:
                padded_position_ids = ops.arange(input_shape[-1], padded_seq_length, dtype=ms.int64)
