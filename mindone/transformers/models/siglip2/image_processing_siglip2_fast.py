...
def convert_image_to_patches(image: "ms.Tensor", patch_size: int) -> "ms.Tensor":
    """
...
def pad_along_first_dim(
    tensor: "ms.Tensor", target_length: int, pad_value: int = 0
    ...
    mask = mint.ones((target_length,), dtype=ms.int32)
    if padding_length > 0:
        padding = [0, 0] * (tensor.ndim - 1) + [0, padding_length]
        tensor = mint.nn.functional.pad(tensor, padding, mode="constant", value=pad_value)
        ...
        **kwargs,
        ...
        for image in images:
            if do_resize:
                ...
                image = ms.tensor(np.array(image)).transpose(2, 0, 1)
        ...
        pixel_values = mint.stack(pixel_values)
        pixel_masks = mint.stack(pixel_masks)
