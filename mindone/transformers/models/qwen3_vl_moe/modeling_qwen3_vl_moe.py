...
class Qwen3VLMoeTextExperts(nn.Cell):
    def __init__(self, config):
        ...
        self.gate_up_proj = Parameter(
            ms.tensor(np.ones((self.num_experts, self.hidden_size, 2 * self.expert_dim)), dtype=config.dtype if config.dtype else ms.float32)
        )
        self.down_proj = Parameter(
            ms.tensor(np.ones((self.num_experts, self.expert_dim, self.hidden_size)), dtype=config.dtype if config.dtype else ms.float32)
        )
