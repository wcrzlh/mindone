...
class TimesformerEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.position_embeddings = Parameter(ops.zeros((1, self.num_patches + 1, embed_dim)))
        self.pos_drop = nn.Dropout(p=drop_rate)
        if attention_type != "space_only":
            self.time_embeddings = Parameter(ops.zeros((1, num_frames, embed_dim)))
...
class TimesformerPreTrainedModel(MSPreTrainedModel):
    """
