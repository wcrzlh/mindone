...
class Resampler(nn.Cell):
    ...
    def _set_2d_pos_cache(self, max_size, device='cpu'):
        if is_deepspeed_zero3_enabled():
            device='cuda'
        pos_embed = ms.numpy(get_2d_sincos_pos_embed(self.embed_dim, max_size)).float()
        self.register_buffer("pos_embed", pos_embed, persistent=False)

    def _adjust_pos_cache(self, tgt_sizes, device):
        max_h = ops.max(tgt_sizes[:, 0])
        max_w = ops.max(tgt_sizes[:, 1])
    ...
    def construct(self, x, tgt_sizes=None):
        ...
        max_patch_len = ops.max(patch_len)
        key_padding_mask = ops.zeros((bs, max_patch_len), dtype=ms.bool)
...
class MultiheadAttention(nn.MultiheadAttention):
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, 
        ...
        self.out_proj = nn.Dense(embed_dim, embed_dim, has_bias=bias, dtype=dtype)
    ...
    def multi_head_attention_forward(
        ...
        if need_weights:
            ...
            if attn_mask is not None:
                attn_output_weights = ops.baddbmm(attn_mask, q_scaled, k.transpose(-2, -1))
            else:
                attn_output_weights = ops.bmm(q_scaled, k.transpose(-2, -1))
...
def _canonical_mask(
    ...
    if mask is not None:
        ...
        if _mask_dtype != ms.bool and not _mask_is_float:
            raise AssertionError(
