...
from mindone.transformers.mindspore_adapter import dtype_to_min, dtype_to_max
...
class LogitsProcessorTest(unittest.TestCase):
    ...
    #
    #     # compute softmax
    #     probs = mint.nn.functional.softmax(scores, dim=-1)
    #
    ...
    #
    #     warped_prob_sharp = mint.nn.functional.softmax(temp_dist_warper_sharper(input_ids, scores), dim=-1)
    #     warped_prob_smooth = mint.nn.functional.softmax(temp_dist_warper_smoother(input_ids, scores), dim=-1)
    ...
    def test_top_k_dist_warper(self):
        ...
        ramp_logits = (
            mint.arange(vocab_size, dtype=ms.float32).unsqueeze(0).repeat(batch_size, 1)
        )
    ...
    #     dist = mint.log(
    #         ms.tensor([[0.3, 0.1, 0.1, 0.5], [0.15, 0.3, 0.3, 0.25]], dtype=ms.float32)
    #     )
    #
    #     top_p_warp = TopPLogitsWarper(0.8)
    #     filtered_dist = mint.exp(top_p_warp(input_ids, dist))
    #
    #     # dist should be filtered to keep min num values so that sum is >= top_p
    #     # exp (-inf) => 0
    #     EXPECTED_FILTERED_DIST = ms.tensor(
    #         [[0.3, 0.0, 0.0, 0.5], [0.0, 0.3, 0.3, 0.25]], dtype=ms.float32
    #     )
    ...
    #     filtered_dist = mint.exp(min_p_warp(input_ids, dist))
    #
    #     # exp (-inf) => 0
    #     EXPECTED_FILTERED_DIST = ms.tensor(
    ...
    def test_no_repeat_ngram_dist_processor(self):
        ...
        self.assertListEqual(
            mint.isinf(filtered_scores_3_gram).tolist(), [[False, False, False], [True, False, False]]
        )
    ...
    def test_encoder_no_repeat_ngram_dist_processor(self):
        ...
        self.assertListEqual(
            mint.isinf(filtered_scores_3_gram).tolist(), [[False, True, False], [False, False, False]]
        )
    ...
    def test_hamming_diversity(self):
        ...
        self.assertTrue(
            mint.allclose(
                processed_scores[0], ms.tensor([-0.7500, 0.2500, 0.2500, 0.2500]), atol=1e-3
            )
        )
        self.assertTrue(
            mint.allclose(
                processed_scores[1], ms.tensor([0.2500, -0.7500, 0.2500, 0.2500]), atol=1e-3
            )
        )
    ...
    def test_normalization(self):
        ...
        scores = ms.tensor(
            [[-23.18, -29.96, -43.54, 47.77], [-33.58, -26.87, -32.96, 22.51]], dtype=ms.float32
        )
