...
from mindone.transformers import AutoConfig, AutoProcessor, pipeline
...
class GenerationTesterMixin:
    ...
    # fix me there is no implementation for group_beam_search
    # @pytest.mark.generate
    ...
    def test_generate_with_head_masking(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            head_masking = {
                "head_mask": mint.zeros(
                    (text_config.encoder_layers, text_config.encoder_attention_heads)
                ),
                "decoder_head_mask": mint.zeros(
                    (text_config.decoder_layers, text_config.decoder_attention_heads)
                ),
                "cross_attn_head_mask": mint.zeros(
                    (text_config.decoder_layers, text_config.decoder_attention_heads)
                ),
            }
    ...
    def test_left_padding_compatibility(self):
        ...
        def _prepare_model_kwargs(input_ids, attention_mask, signature):
            ...
            if "position_ids" in signature:
                position_ids = mint.cumsum(attention_mask, dim=-1) - 1
                position_ids.masked_fill_(attention_mask == 0, 1)
                model_kwargs["position_ids"] = position_ids
            if "cache_position" in signature:
                cache_position = mint.arange(input_ids.shape[-1])
        ...
        for model_class in decoder_only_classes:
            ...
            padding = mint.ones(pad_size, dtype=input_ids.dtype) * pad_token_id
            padded_input_ids = mint.cat((padding, input_ids), dim=1)
            padded_attention_mask = mint.cat((mint.zeros_like(padding), attention_mask), dim=1)
            model_kwargs = _prepare_model_kwargs(padded_input_ids, padded_attention_mask, signature)
            next_logits_with_padding = model(**model_kwargs).logits[:, -1, :]

            # They should result in very similar logits
            # fixme there is no mindspore testing
    ...
    #
    #         main_input = inputs_dict[model.main_input_name].set_train(False)
    ...
    #                 input_1[key] = value[:half_batch_size, :].set_train(False)
    #                 input_2[key] = value[half_batch_size : half_batch_size * 2, :].set_train(False)
    #             else:
    #                 input_1[key] = value
    #                 input_2[key] = value
    ...
    #         # fixme how to align with torch.compiler
...
class GenerationIntegrationTests(unittest.TestCase):
    # fixme there is no implementation for group_beam_search
    # @slow
    ...
    #     input_ids = ms.tensor(bart_tokenizer(article, return_tensors="np").input_ids)
    #
    ...
    # # fixme fixbugs caused by gpt2/bart/whisper/cache_XXX
    ...
    #     article = """Justin Timberlake and Jessica Biel, welcome to parenthood."""
    #     bart_tokenizer = BartTokenizer.from_pretrained("sshleifer/bart-tiny-random", revision="refs/pr/1")
    #     bart_model = BartForConditionalGeneration.from_pretrained("sshleifer/bart-tiny-random", revision="refs/pr/1")
    #
    #     input_ids = ms.tensor(bart_tokenizer(article, return_tensors="np").input_ids)
    ...
    #     generator = pipeline("text-generation", model="hf-internal-testing/tiny-random-bart", revision="refs/pr/1")
    ...
    #         "hf-internal-testing/tiny-random-imagegpt", revision="refs/pr/1", max_length=10
    #     )
    ...
    #     ]
    #     tokenizer = BartTokenizer.from_pretrained("hf-internal-testing/tiny-random-bart", revision="refs/pr/1")
    ...
    #     )
    #
    #     input_ids = ms.tensor(tokenizer(articles, return_tensors="np", padding=True).input_ids)
    ...
    #     model_inputs = ms.tensor(tokenizer("I will be", return_tensors="np"))
    ...
    #     logits_processor = watermark_config.construct_processor(model.config.vocab_size)
    #     mean_g_values_repeats = []
    #     for _ in range(40):
    #         input_ids = mint.zeros(
    ...
    #
    #     mean_g_values = mint.concat(mean_g_values_repeats, dim=0).mean(dim=0)
    #     expected_mean_g_value = logits_processor.expected_mean_g_value(
    #         vocab_size=model.config.vocab_size,
    #     )
    #     atol = 0.03
    #     is_close = mint.isclose(
    #         mean_g_values,
    #         ms.tensor(expected_mean_g_value, dtype=ms.float64),
    #         atol=atol,
    #         rtol=0,
    #     )
    #     self.assertTrue(mint.all(is_close))
    ...
    #
    #     input_ids = ms.tensor(tokenizer(starting_text, return_tensors="np").input_ids)
    #
    #     outputs = model.generate(
    #         input_ids,
    #         constraints=constraints,
    ...
    # fixme there is no implementation for constrained beam search
    # @slow
    ...
    #     tokenizer = GPT2Tokenizer.from_pretrained("openai-community/gpt2")
    #
    #     input = tokenizer(["The dragon flew over Paris,"], return_tensors="np", return_attention_mask=True)
    #     input["input_ids"] = ms.tensor(input["input_ids"])
    #     input["attention_mask"] = ms.tensor(input["attention_mask"])
    #
    ...
    #     )
    #
    #     neg = tokenizer(["France,"], return_tensors="np", return_attention_mask=True)
    #     neg["input_ids"] = ms.tensor(neg["input_ids"])
    #     neg["attention_mask"] = ms.tensor(neg["attention_mask"])
    #     outputs = model.generate(
    #         **input,
    #         max_new_tokens=32,
    #         guidance_scale=1.5,
    ...
    #     tokenizer.pad_token_id = tokenizer.eos_token_id
    #     input_ids = ms.tensor(tokenizer(text, return_tensors="np", padding="longest", add_special_tokens=False).input_ids)
    #
    ...
    # fixme group_beam_search is not implemented
    # def test_constrained_beam_search_mixin_type_checks(self):
    #     tokenizer = AutoTokenizer.from_pretrained("patrickvonplaten/t5-tiny-random", revision="refs/pr/4")
    #     model = AutoModelForSeq2SeqLM.from_pretrained("patrickvonplaten/t5-tiny-random", revision="refs/pr/4")
    #
    #     encoder_input_str = "translate English to German: How old are you?"
    #     input_ids = ms.tensor(tokenizer(encoder_input_str, return_tensors="np").input_ids)
    #
    #     with self.assertRaises(ValueError):
    #         force_words = ["sind"]
    #         force_words_ids = ms.tensor(tokenizer(force_words, return_tensors="np").input_ids)
    #         model.generate(
    #             input_ids,
    #             force_words_ids=force_words_ids,
    #             num_beams=10,
    ...
    #         "Michael Phelps is arguably the most decorated Olympian of all time.",
    #     ]
    #     bart_tokenizer = AutoTokenizer.from_pretrained("hf-internal-testing/tiny-random-bart", revision="refs/pr/1")
    #     bart_model = BartForConditionalGeneration.from_pretrained("hf-internal-testing/tiny-random-bart", revision="refs/pr/1")
    #     input_ids = ms.tensor(bart_tokenizer(articles, return_tensors="np", padding=True).input_ids)
    ...
    #     tokenizer = BartTokenizer.from_pretrained("hf-internal-testing/tiny-random-bart", revision="refs/pr/1")
    #     model = BartForConditionalGeneration.from_pretrained("hf-internal-testing/tiny-random-bart", revision="refs/pr/1")
    #
    #     model.config.eos_token_id = None
    #     input_ids_batched = ms.tensor(tokenizer(articles, padding=True, return_tensors="np").input_ids)
    #     input_ids = ms.tensor(tokenizer(articles[1], return_tensors="np").input_ids)
    #
    ...
    #         tokens[key] = ms.tensor(tokens[key])
    #     model = AutoModelForCausalLM.from_pretrained("hf-internal-testing/tiny-random-gpt2")
    #
    ...
    #     # fixme how to align with torch random seed
    #     # torch.manual_seed(1)
    #     eos_token_id = 846
    ...
    #
    #     bart_model = FakeBart.from_pretrained("hf-internal-testing/tiny-random-bart", revision="refs/pr/1")
    ...
    #     input_ids = ms.tensor(tokenized_inputs.input_ids)
    #
    ...
    # fixme there is no implementation for assisted_generation
    ...
    #         input_ids,
    #         token_type_ids=mint.zeros(input_ids.shape, dtype=ms.int64),
    #     )
    ...
    #
    #     # process the input:
    #     features = processor(inputs, return_tensors="np")
    #     for key in features.keys():
    #         features[key] = ms.tensor(features[key])
    #
    ...
    #     probs_all = mint.nn.functional.softmax(outputs.logits[2][0], dim=-1)
    ...
    def test_special_tokens_fall_back_to_model_default(self):
        model = AutoModelForCausalLM.from_pretrained("hf-internal-testing/tiny-random-MistralForCausalLM", revision="refs/pr/29")
        test_bos_id = 50
    ...
    #
    #     prompt_size = mint.randint(20, 100, (1,))
    #     max_new_tokens = mint.randint(10, 50, (1,))
    #     input_ids = (mint.rand(1, prompt_size[0]) * 100).to(ms.int32) + 50
    #
    ...
    # fixme there is no implementation for contrasive search
    # @slow
    ...
    #     encoded_prompt = tokenizer(prompt_text, return_tensors="np", padding=True)
    #     input_ids = ms.tensor(encoded_prompt.input_ids)
    ...
    #     padded_input_ids = mint.nn.functional.pad(
    #         input_ids, (padding_length, 0), "constant", value=model.generation_config.pad_token_id
    #     )
    #     padded_attention_mask = mint.nn.functional.pad(attention_mask, (padding_length, 0), "constant", value=0)
    #
    ...
    #
    #     # Tokenize the input prompt
    #     encoded_prompt = tokenizer(prompt_text, return_tensors="np")
    #     input_ids = ms.tensor(encoded_prompt.input_ids)
    #     attention_mask = ms.tensor(encoded_prompt.attention_mask)
    #
    #     # Define the decoder prompt
    #     decoder_prompt_text = "Ich muss diese Aufgabe"
    #     encoded_decoder_prompt = tokenizer(decoder_prompt_text, add_special_tokens=False, return_tensors="np")
    #     decoder_input_ids = ms.tensor(encoded_decoder_prompt.input_ids)
    #     decoder_attention_mask = ms.tensor(encoded_decoder_prompt.attention_mask)
    #
    #     # Define the contrastive search params
    #     penalty_alpha = 0.6
    #     top_k = 4
    ...
    #     padded_decoder_input_ids = mint.nn.functional.pad(
    #         decoder_input_ids, (padding_length, 0), "constant", value=model.generation_config.pad_token_id
    #     )
    #     padded_decoder_attention_mask = mint.nn.functional.pad(decoder_attention_mask, (padding_length, 0), "constant", value=0)
    ...
    #     input_ids = ms.tensor([[1, 2, 3], [4, 5, 6]])
    #     model_inputs = model.prepare_inputs_for_generation(input_ids)
    #     self.assertTrue(mint.all(model_inputs["input_ids"] == input_ids))
    #
    ...
    #     self.assertTrue(mint.all(model_inputs["attention_mask"] == attention_mask))
    ...
    #
    #     cache_position = mint.arange(input_ids.shape[-1], dtype=ms.int64)
    ...
    #     self.assertTrue(mint.all(model_inputs["cache_position"] == cache_position))
    ...
    #         config=config, batch_size=batch_size, max_cache_len=max_cache_len, dtype=ms.float32
    #     )
    ...
    #     init_cache_positions = mint.arange(init_input_ids.shape[-1], dtype=ms.int64)
    ...
    #     decoder_input_ids = ms.tensor([[1, 2, 3], [4, 5, 6]])
    #     model_inputs = model.prepare_inputs_for_generation(decoder_input_ids)
    #     self.assertTrue(mint.all(model_inputs["decoder_input_ids"] == decoder_input_ids))
    #
    ...
    #     decoder_attention_mask = ms.tensor([[1, 1, 1], [1, 1, 1]])
    #     model_inputs = model.prepare_inputs_for_generation(
    #         decoder_input_ids, decoder_attention_mask=decoder_attention_mask
    #     )
    #     self.assertTrue(mint.all(model_inputs["decoder_attention_mask"] == decoder_attention_mask))
    ...
    #     """
    #     model = AutoModelForCausalLM.from_pretrained(
    #         "hf-internal-testing/tiny-random-LlamaForCausalLM", mindspore_dtype=ms.bfloat16
    #     )
    #     tokenizer = AutoTokenizer.from_pretrained("hf-internal-testing/tiny-random-LlamaForCausalLM")
    #
    #     # compile generate
    #     # fixme how to align with torch compile
    ...
    #     model = GPT2LMHeadModel.from_pretrained("openai-community/gpt2")
    #
    #     # fixme how to get the same performance with torch random seed
    #     # torch.manual_seed(0)
    ...
    #     bart_tokenizer = AutoTokenizer.from_pretrained("hf-internal-testing/tiny-random-bart", revision="refs/pr/1")
    #     article = """Justin Timberlake and Jessica Biel, welcome to parenthood."""
    #     bart_model = AutoModelForSeq2SeqLM.from_pretrained("hf-internal-testing/tiny-random-bart", revision="refs/pr/1",min_length=1)
    #     input_ids = ms.tensor(bart_tokenizer(article, return_tensors="np").input_ids)
    #
    ...
    #     model = AutoModelForSeq2SeqLM.from_pretrained("hf-internal-testing/tiny-random-bart", revision="refs/pr/1")
    #     input_ids = ms.tensor(tokenizer(articles, return_tensors="np", padding=True).input_ids)
    #
    #     outputs = model.generate(
    #         input_ids=input_ids,
    ...
    #     )
    #     model.config.eos_token_id = None
    #     input_ids = ms.tensor(tokenizer(articles[0], return_tensors="np").input_ids)
    #     input_ids_batched = ms.tensor(tokenizer(articles, padding=True, return_tensors="np").input_ids)
    #
    #     generate_kwargs = {
    #         "return_dict_in_generate": True,
    ...
    #     batched_out = batched_out.asnumpy()
    #     out = out.asnumpy()
    #
    ...
    #     output_sequences_kwargs = output_sequences_kwargs.asnumpy()
    #     output_sequences = output_sequences.asnumpy()
    #
    ...
    def test_generate_too_many_encoder_kwargs(self):
        ...
        article = "I need input_ids to generate"
        tokenizer = AutoTokenizer.from_pretrained("hf-internal-testing/tiny-random-bart", revision="refs/pr/1")
        model = AutoModelForSeq2SeqLM.from_pretrained("hf-internal-testing/tiny-random-bart", revision="refs/pr/1", max_length=10)
        input_ids = ms.tensor(tokenizer(article, return_tensors="np").input_ids)
        with self.assertRaises(ValueError):
    ...
    #     input_features = floats_tensor((3, 80, 60))
    #     attention_mask = mint.randint(0, 2, input_features.shape)
    #     model = AutoModelForSpeechSeq2Seq.from_pretrained(
    #         "hf-internal-testing/tiny-random-WhisperForConditionalGeneration",
    #         revision="refs/pr/1",
    #     )
    ...
    #     output_sequences_no_mask = output_sequences_no_mask.asnumpy()
    #     output_sequences_with_mask = output_sequences_with_mask.asnumpy()
    #
    ...
    # fixme there is no VisionEncoderDecoderModel right now in mindone.transformers
    ...
    #     conditioning_input = ms.tensor([[10], [10]])  # this should be the 2nd output token, after the BOS token
...
#         self.model_kwargs = {}
#         self.input_ids = mint.randint(1, 10, (1, 9))
